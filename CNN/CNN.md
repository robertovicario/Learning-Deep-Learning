...
Mathematically, for max pooling and average pooling, the operation on a patch $P$ of the input is defined as:
...
where $M(i, j)$ is the maximum value in the patch $P_{i,j}$.

- **Average Pooling:**

$$
A(i, j) = \frac{1}{|P_{i,j}|} \sum_{(m,n) \in P_{i,j}} P_{m,n}
$$

where $A(i, j)$ is the average value in the patch $P_{i,j}$.
